...
namespace osmium
{
	...
	Hook::Hook(uintptr_t pTarget, uintptr_t pDetour) 
	{
		...
		if (!IsSamePage(reinterpret_cast<const PBYTE>(pTarget), reinterpret_cast<const PBYTE>(pDetour))) 
		{
			...
			if (m_hHandle && VirtualProtect(reinterpret_cast<LPVOID>(pTarget), 1, PAGE_EXECUTE_READ | PAGE_GUARD, &m_dwProtect))
				bSuccess = true;
		}
	}
	Hook::~Hook() 
	{
		...
		if (m_hHandle && VirtualProtect(reinterpret_cast<LPVOID>(m_pTarget), 1, m_dwProtect, &dwOldProtect) && RemoveVectoredExceptionHandler(m_hHandle))
			bSuccess = true;
		else
			bSuccess = false;
	}
